...
from SFC_backprop.load_mnist import load_mnist, load_fmnist
...
def generate_input_data(num_trials, input_data='MNIST10', add_bias=False, threshold=0.5):
    ...
    if input_data == 'MNIST10':
        ...
        for i, l in enumerate(mnist_test_labels[0:num_trials]):
            train_output[i, l] = 1
    elif input_data == 'MNIST28':
        mnist_train_data, mnist_test_data, mnist_train_labels, mnist_test_labels = load_mnist(sidelen=28, crop=0)
        if num_trials > 60000:
            for i in range(int(np.ceil((num_trials - 60000) / 60000))):
                mnist_train_data2, mnist_test_data2, mnist_train_labels2, mnist_test_labels2 = load_mnist(sidelen=28,
                                                                                                          crop=0)
                mnist_train_data = np.concatenate((mnist_train_data, mnist_train_data2), axis=0)
                mnist_train_labels = np.concatenate((mnist_train_labels, mnist_train_labels2))
        ...
        for i, l in enumerate(mnist_test_labels[0:num_trials]):
            train_output[i, l] = 1
    elif input_data == 'FMNIST28':
        mnist_train_data, mnist_test_data, mnist_train_labels, mnist_test_labels = load_fmnist(sidelen=28, crop=0)
        if num_trials > 60000:
            for i in range(int(np.ceil((num_trials - 60000) / 60000))):
                mnist_train_data2, mnist_test_data2, mnist_train_labels2, mnist_test_labels2 = load_fmnist(sidelen=28,
                                                                                                           crop=0)
                mnist_train_data = np.concatenate((mnist_train_data, mnist_train_data2), axis=0)
                mnist_train_labels = np.concatenate((mnist_train_labels, mnist_train_labels2))
    ...
    train_input = (threshold < train_input)
